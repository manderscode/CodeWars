...
function isolateIt(arr){
    ...
    for (let i = 0; i < arr.length; i++ ) { 
      ...
      if (arr[i].length % 2 === 0) { 
        const newStr = arr[i].slice(0,arr[i].length/2) + '|' + arr[i].slice(arr[i].length/2)
        console.log('even')
         newArr.push(newStr) //think of this like subject, verb, and what you're doing it with 
      } else { 
        const oddStr = arr[i].slice(0,arr[i].length/2) + '|' + arr[i].slice(arr[i].length/2+1)
        console.log('odd')
        newArr.push (oddStr) 
